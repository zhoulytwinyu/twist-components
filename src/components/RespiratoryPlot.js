...
class RespPlot extends PureComponent {
  constructor(props){
    ...
    this.memo = {};
    this.memo.prevWidth = null;
    this.memo.prevHeight = null;
  ...
  draw() {
    ...
    // Adjust array size
    memo.respiratoryScores_domXs.length = respiratoryScores.length;
    memo.respiratoryScores_RespiratorySupportScoreDomYs.length = respiratoryScores.length;
    memo.respiratoryScores_ECMOScoreDomYs.length = respiratoryScores.length;
    memo.iNO_startDomXs.length = iNO.length;
    memo.iNO_endDomXs.length = iNO.length;
    memo.anesthetics_startDomXs.length = anesthetics.length;
    memo.anesthetics_endDomXs.length = anesthetics.length;
    // Filter data
    ...
    for (let i=iNO_startIndex; i<=iNO_endIndex; i++){
      ...
      memo.iNO_endDomXs[i] = toDomXCoord_Linear(width,minX,maxX,memo.iNO_ends[i]);
    }
    for (let i=anesthetics_startIndex; i<=anesthetics_endIndex; i++){
      ...
      memo.anesthetics_endDomXs[i] = toDomXCoord_Linear(width,minX,maxX,memo.anesthetics_ends[i]);
    }
    // Clear plots
