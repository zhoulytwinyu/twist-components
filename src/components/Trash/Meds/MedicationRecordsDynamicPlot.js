...
class MedicationRecordsDynamicPlot extends PureComponent {
  ...
  draw() {
    if (this.lastMR !== this.props.medicationRecords){
      this.lastMR = this.props.medicationRecords
      console.log("!@#")
    }
    let { medicationRecords,
          categoryStructure,
    ...
    // Filter
    let filteredMedicationRecords = medicationRecords.filter( ({start}) =>
                                                                (start>=minX && start<=maxX)
                                                              )
    // Get position
  ...
  drawRectangle(ctx,startX,endX,y,r,color) {
    ...
    let w = Math.round(endX) - startX
    let startY = Math.round(y-r)
    let endY = Math.round(y+r)
    let h = endY-startY
    ctx.fillStyle = color
    ctx.strokeStyle = "black"
